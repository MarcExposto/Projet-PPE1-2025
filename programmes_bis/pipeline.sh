...
echo "id \t url \t gestion de robots.txt \t code http \t encodage initial de la page \t lien vers la page brute \t statut de la conversion en UTF-8 \t lien vers le dump textuel \t numbre total de mots dans le dump \t nombre d'occurrences du mot cible dans le dump \t lien vers le concordancier \t lien vers l'analyse des bigrammes"
...
while read -r line; do
  ...
  if [[ -n $ErreurConversion ]]; then
    echo "${id}\t${Url}\t${Robots}\t${CodeHTTP}\t${Encodage}\tÉchec\t\t\t\t\t\t"
    ((id++))
    continue
  fi
  ...
  NbOccurrences=$(egrep ${MotCible} ${CheminFichierTexte})
  ...
  echo "${id}\t${Url}\t${Robots}\t${CodeHTTP}\t${Encodage}\tConverti\t[Lien vers le dump](${CheminFichierTexte})\t${NbMots}\t${NbOccurrences}\t[Lien vers le fichier tokenisé](${CheminFichierTokens})\t[Lien vers le concordancier](${CheminFichierConcordances})\t[Lien vers les bigrammes](${CheminFichierBigrammes})"
